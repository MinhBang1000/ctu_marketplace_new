...
import './NAdmin-Manage-Footer.scss'
import axios from "axios";
import nl2br from 'react-nl2br';
import NFooter from "../../../components/NFooter";

function NAdminManageFooter() {
    const [footers, setFooters] = useState([])
 
    const [backupFooters, setBackupFooters] = useState([]);

    const [updateFooterLoading, setUpdateFooterLoading] = useState(false);
    const [savedFooter, setSavedFooter] = useState(false);

    const [editingCheck, setEditingCheck] = useState(false);

    const [idFooter, setIdfooter] = useState();
    const api = 'https://marketplace.ctu.edu.vn/api/v3/'

    useEffect(() => {
        axios.get(`${api}footers`)
            .then(res => {
                setIdfooter(res.data.data[0].id);
                console.log('Footer', typeof idFooter)
                setFooters([...res.data.data[0].newFooterInfos]);
                setBackupFooters([...res.data.data[0].newFooterInfos]);
            })
            .then(res => {
    ...
    const handleAddFooter = (e) => {
        e.preventDefault();
        setFooters([...footers, { footerKey: '', footerValue: '' }]);
        setEditingCheck(true);
    }

    const handleUpdateFooterValue = (e, index) => {
        var nFooters = [...footers];
        nFooters[index].footerValue = e.target.value;
        setFooters(nFooters);
        setEditingCheck(true);
        console.log(footers);
        console.log(backupFooters)
    }

    const handleUpdateFooterName = (e, index) => {
        var nFooters = [...footers];
        nFooters[index].footerKey = e.target.value;
        setFooters(nFooters);
        setEditingCheck(true);
    }

    const handleDeleteFooter = (e, indexFooter) => {
        var nFooters = [...footers];
        nFooters = nFooters.filter((footer, index) => {
            return index !== indexFooter
        })
        setFooters(nFooters);
        setEditingCheck(true);
    ...
    const handleUpdateFooter = (e) => {
        ...
        console.log("eee: ", idFooter)
        if (editingCheck === true) {
            setUpdateFooterLoading(true);
            axios.put(`${api}footers/${idFooter}`, {
                name: "Footer 1.5",
                domainId: 999, newFooterInfos: footers
            })
                .then(res => {
                    console.log("res: ", res);
                    setUpdateFooterLoading(false);
                    setBackupFooters([...footers]);
                    setEditingCheck(false);

                    NFooter.setFooters([...footers]);
                })
                .then(() => {
                    setSavedFooter(true);
                    setTimeout(() => {
    ...
    return (
        ...
        <div className='management-footer'>
            <div className='management-footer__modify'>
                <div className='none' style={{ height: '25px' }}></div>
                <div className='management-footer__modify__field-list'>
                    {
                        footers.map((footer, index) => {
                            return (
                                <div key={index} className='management-footer__modify__field-list__item'>
                                    <div className='management-footer__modify__field-list__item__title'>
                                        <input
                                            value={footer.footerKey}
                                            onChange={(e) => handleUpdateFooterName(e, index)}
                                            placeholder='Tiêu đề'
                                        />
                                        <div className='remove-icon' onClick={(e) => handleDeleteFooter(e, index)}>
                                    ...
                                    <textarea className='management-footer__modify__field-list__item__value'
                                        value={footer.footerValue}
                                        onChange={(e) => handleUpdateFooterValue(e, index)}
                ...
                <div className='management-footer__modify__add-new'>
                    {
                        footers.length <= 3 ?
                        <button onClick={(e) => handleAddFooter(e)}>
                        Thêm trường mới
                    ...
                    : <span style={{ color: 'green' }}>Đạt trường tối đa</span>
                    }
                </div>
                <div className='management-footer__modify__button'>
                    <div className={!savedFooter ? 'management-footer__modify__button__notification' : 'management-footer__modify__button__notification management-footer__modify__button__notification--active'}>
                        Đã lưu
                    </div>
                   
                   
                    <div className='management-footer__modify__button__update' onClick={(e) => handleUpdateFooter(e)} id='review'>
                        {
                            updateFooterLoading ? <div class="spinner-border" role="status"><span class="sr-only">Loading...</span> </div>
                                :
                                <span>
                                    Lưu
...
export default NAdminManageFooter